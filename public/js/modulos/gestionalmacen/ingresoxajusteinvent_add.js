$( document ).ready(function() { 
  ...
  if($('#form_save_ingresoxajusteinvent').length)
  {
    $('#form_save_ingresoxajusteinvent').validate({
      rules:
      {
      ...
      submitHandler: function() {
        ...
        {
          $.ajax({
            url: $('base').attr('href') + 'ingresoxajusteinvent/save_ingresoxajusteinvent',
            type: 'POST',
            data: $('#form_save_ingresoxajusteinvent').serialize(),
            dataType: "json",
            beforeSend: function() {
            },
            success: function(response) {
              if (response.code==1) {
              var url = $('base').attr('href') +$('#url_modulo').val()+'/edit/'+response.data.id+'/imputaciones';
  ...
  $('#form_save_salidaxajusteinvent').validate({
    rules:
    {
      fecha_ingreso:{ required:true},
      exist_pedido:{ required:true }
    ...
    submitHandler: function() {
      ...
      {
        $.ajax({
          url: $('base').attr('href') + 'salidaxajusteinvent/save_ingresoxajusteinvent',
          type: 'POST',
          data: $('#form_save_salidaxajusteinvent').serialize(),
          dataType: "json",
          beforeSend: function() {
          },
          success: function(response) {
...
function valid_exist_pedido()
{
  ...
  var ialm = 0;
  var sto = 0;
  var idtipo = parseInt($('#id_tipo').val());
...
$(document).on('click', '.limpiarfiltro', function (e) {
  $('#buscarart thead input[type="text"]').val('');
  var page = 0;
  buscar_articulos(page);
});
...
function buscar_articulos(page)
{
  ...
  $.ajax({
    url: $('base').attr('href') + 'ingresoxajusteinvent/buscar_articulos',
    type: 'POST',
    data: temp,
    dataType: "json",
    beforeSend: function() {
...
$(document).on('click', 'a.edit_articulo', function (e)
{
  ...
  var temp = 'id_art_sucursal='+idsu+'&orden='+orden+'&id_tipo='+tipo;

  $.ajax({
    url: $('base').attr('href') + 'ingresoxajusteinvent/add_articulo',
    type: 'POST',
    data: temp,
    dataType: "json",
    beforeSend: function() {
...
$(document).on('click', 'a.add_arti', function (e)
{
  ...
  var temp = 'id_art_sucursal='+idsu+'&orden='+orden+'&edit_orden=1'+'&id_tipo='+tipo;
