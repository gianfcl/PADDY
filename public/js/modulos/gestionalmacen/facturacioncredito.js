  ...
  $('#form_add_periodocredito').validate({
    rules:
      ...
      submitHandler: function() {
        ...
        var fuck = "facturacion";
        if($('#form_save_ingresoxcompra').length)
        {
          fuck = "ingresoxcompra";
        }

        if($('#form_save_registrogasto').length)
        {
          fuck = "registrogasto";
        }

        $.ajax({
          url: $('base').attr('href') + fuck+'/add_periodos',
          type: 'POST',
...
$(document).on('change', '#id_formapago', function (e) {  
  ...
  if(id==2)
  {
    if($('#lista_articulos tbody tr.ordenes td.orden').length)
    {
...
$(document).on('change', '#id_periodocredito_det', function (e) {
  var periodo = parseInt($('#id_periodocredito_det option:selected').val()); console.log(periodo);
...
function get_joinpvett(div)
{
  ...
  var ids =  new Array();
  var i = 0;
  $('#'+div+' tbody#bodyindex tr.ordenes td.'+fuck+' input').each(function (index, value){
    ids[i] = $(this).val();
    i++;
  });
  return ids.join(',');
