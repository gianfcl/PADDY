$( document ).ready(function() {
  ...
  $.validator.addMethod("regex", function(value, element) {
    var sal = parseInt($('#id_saldo').html());
    var exp = value;
    console.log(sal);
    if (exp <= 0) { return false; }
    else {
      if($.isNumeric(exp)){ 
        if(exp <= sal){return true; }else{return false; } 
      }
      else{ return false; }
    }
  }, "Solo numeros Mayores a 0 y menor al Saldo" );
  ...
  $('#form_nuevoingreso').validate({
    ...
    submitHandler: function() {
      $.ajax({
        url: $('base').attr('href') + 'salidaxajustecuent/save_salidaxajustecuenta',
        type: 'POST',
        ...
        success: function(response) {
          if (response.code==1) {
            ...
            buscar_salidaxajustecuenta(page);
          }
          else
          {
            limpiarform();
...
function buscar_salidaxajustecuenta(page)
{
  ...
  console.log(temp);
  $.ajax({
      url: $('base').attr('href') + 'salidaxajustecuent/buscar_salidaxajustecuentas',
      type: 'POST',
      data: temp,
      dataType: "json",
      beforeSend: function() {
...
$(document).on('change', '#id_moneda', function (e) {
  ...
  if(idmon>0)
  {
    ...
    $.ajax({
      url: $('base').attr('href') + 'salidaxajustecuent/cbx_cuentadetin',
      type: 'POST',
      data: temp,
      dataType: "json",
      beforeSend: function() {
...
$('#uno').click(function(){
$('#verform_nuevoingreso').show();
$('#verform_sis').hide();
});

$('#dos').click(function(){
$('#verform_sis').show();
$('#verform_nuevoingreso').hide();
});
...
$(document).on('click', '.ver_obser', function (e) {
  ...
  {
    $.ajax({
      url: $('base').attr('href') + 'salidaxajustecuent/ver_observacion',
      type: 'POST',
      data: 'id_nuevoingreso='+idcob,
      dataType: "json",
      beforeSend: function() {
