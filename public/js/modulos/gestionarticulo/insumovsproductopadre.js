...
{	
	...
	$.ajax({
		url: $('base').attr('href') + 'insumovsproductopadre/buscar_articulos',
      	type: 'POST',
      	data: param,
      	dataType: "json",
      	beforeSend: function() {
...
$(document).on('click','.gen_reporte',function(){
	...
	$.ajax({
      url: $('base').attr('href') + 'insumovsproductopadre/generar_reporte',
      type: 'POST',
      ...
      success: function(response) {
        if (response.code==1) {
          ...
          $("#arti").html(response.data.arti.descripcion);
