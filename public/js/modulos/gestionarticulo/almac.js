...
$(document).on('change', '.id_frec', function (e) {
  var idubi = parseInt($(this).val());
  idubi = (idubi>0) ? (idubi) : (0);
  var padre = $(this).closest('.contenedor');
  var idartalm = parseInt(padre.find('input.id_arti_almacen').val());
  ...
  if(idubi>0 && idartalm>0) {
    $.ajax({
      url: $('base').attr('href') + 'articuloxsucursal/save_frecuencia',
      type: 'POST',
      data: 'id_frecuenciainventfisico='+idubi+'&id_arti_almacen='+idartalm,
      dataType: "json",
      beforeSend: function() {
        //showLoader();
      },
