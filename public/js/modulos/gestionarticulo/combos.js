...
$(document).on('focusout', '.precio_v', function (e)
{
  var idartisucucombos = parseInt($(this).parents('tr').attr('idartisucucombos'));
  var precio_venta = $(this).val();
  if(idartisucucombos >0) 
  {
    var temp = "id_artisucucombos="+idartisucucombos+"&precio_venta="+precio_venta;      
    $.ajax({
      url: $('base').attr('href') + 'articuloxsucursal/edit_combo',
      type: 'POST',
      data: temp,
      dataType: "json",
      beforeSend: function() {
...
$(document).on('change', '#escombopreciovariable', function (e) {
  var es_pv = ($(this).is(':checked')) ? 1: 0;
  es_pv = parseInt(es_pv);
  var id = $('#id_art_sucursal').val();
  
  if(id>0)
  {
    $.ajax({
      url: $('base').attr('href') + 'articuloxsucursal/save_comboprevariable',
      type: 'POST',
      data: 'id_art_sucursal_padre='+id+'&es_pv='+es_pv,
      dataType: "json",
      beforeSend: function() {
          //showLoader();
      },
      ...
      complete: function() {
        if(es_pv == 1)
          $('#div_es_combo table .p_adicional').removeClass('collapse');
        else
          $('#div_es_combo table .p_adicional').addClass('collapse');
      }
    });
  }
});
