$( document ).ready(function() {
	...
	$('form#form_save_reserva input').each(function(index,value){
		  //alert(value.id);
		  $(this).prop('disabled',true);		
	});
	$('form#form_save_reserva select').each(function(index,value){
		  //alert(value.id);
		  $(this).prop('disabled',true);		
	});
	$('form#form_save_reserva a').each(function(index,value){
		  //alert(value.id);
		  $(this).addClass('hidden');		
	});
	$('form#form_save_reserva textarea').each(function(index,value){
		  //alert(value.id);
		  $(this).prop('disabled',true);	
	});
	...
	/* id_tipoevento = $('#id_tipoevento').val();
	if(id_tipoevento==0){}
	else{
...
$(document).on('change', '#no_anulados', function (e) {
  var page = 0;
  buscar_reservas(page);
});
...
{ 
	...
	var temp = "page="+page;
	if($('#no_anulados').is(':checked'))
  	{
    temp=temp+'&no_anulados=1';
  	}
	if($('#para_hoy').is(':checked'))
  	{
    temp=temp+'&desde_hoy=1';
  	}
	...
	$.ajax({
			url: $('base').attr('href') + 'reservaver/buscar_reservas',
			type: 'POST',
			data: temp,
			dataType: "json",
			beforeSend: function() {
