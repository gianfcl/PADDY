    ...
    $('#form_save_posicion').validate({
        rules:
        {
          ...
          orden:{
            required:true,
            remote: {
              url: $('base').attr('href') + 'posicion/validar_orden',
              type: "post",
              data: {
                orden: function() { return $( "#orden" ).val(); },
                id_areaposicion: function() { return $('#id_areaposicion').val(); },
                id_posicion: function() { return $('#id_posicion').val(); }
              }
            }
...
function buscar_posicions(page)
{
  ...
  var abreviatura = $('#abreviatura_busc').val();
  var orden = $('#orden_busc').val();
  ...
  if(estado>-1)
  {
    temp=temp+'&estado='+orden;
  }
  if(orden.trim().length)
  {
    temp=temp+'&orden='+orden;
  }
  if(um_busc1.trim().length)
  {
    temp=temp+'&um_busc1='+um_busc1;
  }
  if(abreviatura.trim().length)
  {
    temp=temp+'&abreviatura='+abreviatura;
  }
...
$(document).on('click', '.edit', function (e) {
  var idposicion = parseInt($(this).parents('tr').attr('idposicion'));
  if(idposicion>0)
  {
    $.ajax({
