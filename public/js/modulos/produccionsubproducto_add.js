    ...
    submitHandler: function() {
      ...
      if(cpu>0)
      {
        ...
        if(stock>=canti)
        {
          ...
          {
            $.ajax({
              url: $('base').attr('href') + 'produccionsubproducto/save_produccionsubproducto',
              type: 'POST',
              ...
              success: function(response) {
                if (response.code==1) 
                {
                  window.location.href = $('base').attr('href') +'produccionsubproducto/ver_documento/'+response.data.id;
                }
              },
        ...
        {
          $('#cantidad').addClass('erroinput');
          alerta('Verificar!', 'Verificar el Stock', 'error');
        }          
      }
      else
      {
...
$(document).on('click', '.add_arti', function (e)
{
  ...
  window.location.href = $('base').attr('href') +'produccionsubproducto/add/'+id_art_sucursal;
});
...
$(document).on('focusout', '.catsubprod', function (e) {
  ...
  var padre = $(this);
  var cant = padre.val(); //console.log('cantidad->'+cant);
  var valid = validnum(cant);
  var cantotal = $('.copu').val();

  if(valid) {
    var id = $(this).closest('tr').attr('id');
    var ialm = parseInt($('tr#'+id+' td.almace select.almaingreso').val());
    var validal = validnum(ialm);
    if(validal>0)
    {
      var c_u = parseFloat($('tr#'+id+' td.almace input.c_u_'+ialm).val()); console.log('costo_u->'+c_u);
      var old = parseFloat($('tr#'+id+' td.cp_t span').html());
      var tot = parseFloat($('b#total').html());

      $('tr#'+id+' td.arti input.cpun_'+id).val(c_u);

      var fu = parseFloat($('tr#'+id+' td.arti input.fu_'+id).val()); console.log('factoru->'+fu);
      var ft = cant*fu; 

      ft = (validnum(ft)) ? (ft.toFixed(2)) : ("0.00");

      $('tr#'+id+' td.factort span').html(ft);
...
$(document).on('change', '.almapartida', function (e) {
  ...
  if(ialm>0)
  {
    ...
    $('table#alma_llegada tbody tr td.cp_u span').html(sum1);
    $('table#alma_llegada tbody tr td.cp_t span').html(sum2);
...
$(document).on('change', '#id_salida', function (e) {
  ...
  {
    padre.closest('tr').find('td.cp_u span').html('0.00');
    padre.closest('tr').find('td.cp_t span').html('0.00');
