$( document ).ready(function() {
  ...
  var fech = $('#fechainix').val();
  var fechavalfin = $('#fechavalfin').val();
  ...
  var fechainix = $('#fechainix').val();
  fechainix = moment(fechainix,'DD-MM-YYYY').format('DD-MM-YYYY');
  var fechavalfin = $('#fechavalfin').val();
  //fechavalfin = moment(fechavalfin,'DD-MM-YYYY').format('DD-MM-YYYY');
  var fexa = $('#fexa').val();
  var miFecha = new Date(fexa);
  fexa = moment(fexa,'DD-MM-YYYY').format('DD-MM-YYYY'); //console.log(fechavalfin);

  $('#fecha_fin').datetimepicker({
    format: 'DD-MM-YYYY',
    locale: moment.locale("es"),
    /*minDate: moment(fechainix,'DD-MM-YYYY').format('DD-MM-YYYY'),
    maxDate: moment(fechavalfin,'DD-MM-YYYY').format('DD-MM-YYYY'),*/
    useCurrent: false
  });
  
  $('#fecha_fin').data("DateTimePicker").minDate(fechainix);
  $('#fecha_fin').data("DateTimePicker").maxDate(fexa);
  $('#fecha_inicio').data("DateTimePicker").maxDate(fexa);
  ...
  $("#fecha_inicio").on("dp.change", function (e) {
    ...
    /*if(moment(fexa).isAfter(moment(e.date,'YYYY-MM-DD')))
    {
      $('#fecha_fin').data("DateTimePicker").minDate(fexa);
      $('#fecha_fin').data("DateTimePicker").maxDate(fexa); //alert("Mayor")
      $('#fecha_fin').data("DateTimePicker").date(fexa);
    }    
    else
    {
      $('#fecha_fin').data("DateTimePicker").date(e.date);
      $('#fecha_fin').data("DateTimePicker").minDate(e.date);
      $('#fecha_fin').data("DateTimePicker").maxDate(e.date.add(1,'M')); alert("Menor")
  ...
  $('#form_save_reporte').validate({
    ...
    submitHandler: function() {
      ...
      $.ajax({
        url: $('base').attr('href') + 'evolucioncmodxop/buscar',
        type: 'POST',
