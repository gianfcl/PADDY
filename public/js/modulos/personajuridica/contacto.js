$( document ).ready(function() {
  ...
  $('#form-add-contactos-nat').validate({
    ...
    submitHandler: function() {
      var orden_ = 0;
      var orden = 0;
      var id_perj = parseInt($('#id_persona_juridica').val());
      var tipo = parseInt($('#tipo_persona').val());

      //alerta('Ok',"lo formamos-->"+orden+'id_sucursal-->'+id_sucursal,'success');
      if(id_perj>0)
      {            
        $.ajax({
            url: $('base').attr('href') + 'personajuridica/save_contacto_nat',
            type: 'POST',
            data: $('#form-add-contactos-nat').serialize()+'&id_persona_juridica='+id_perj+'&tipo_persona='+tipo,
            dataType: "json",
            beforeSend: function() {
              //showLoader();
            },
...
$(document).on('click', '.delete_contacte', function (e)
{
  ...
  swal({
    ...
    {
      $.ajax({
          url: $('base').attr('href') + 'personajuridica/delete_contacto_nat',
          type: 'POST',
          data: 'id_contactos_cliente='+idcontacto,
          dataType: "json",
          beforeSend: function() {
