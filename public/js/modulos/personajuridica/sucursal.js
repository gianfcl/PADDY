$( document ).ready(function() {
  ...
  $( "#dni_cont" ).autocomplete({
    ...
    onSelect: function (suggestion) 
    {
      ...
      var obj = isSession(suggestion.id_persona);
    }
  });
  ...
  $('#form-add-contactos').validate({
    ...
    submitHandler: function() {
      ...
      var id_sucursal = (!isNaN(parseInt($(document).data('datsucu')))) ? $(document).data('datsucu') : 0;
      //alerta('Ok',"lo formamos-->"+orden+'id_sucursal-->'+id_sucursal,'success');
      if(id_sucursal!=0)
      {            
        $.ajax({
            url: $('base').attr('href') + 'personajuridica/save_contacto',
            type: 'POST',
...
$(document).on('click', '.edit_contact', function (e)
{
    ...
    var idsucu = parseInt($(this).parents('.row_sucursal').find('a.cont_modal').attr('idsucu'));
    console.log(idsucu);
    if(idsucu>0)
    {
      $(document).data('datsucu',idsucu);
    ...
    $.ajax({
      ...
      success: function(response) {
        if (response.code == "1") {
          $('#dni_cont').val(response.data.dni);
...
$(document).on('click', '.delete_contact', function (e)
{
  ...
  swal({
    ...
    {
      $.ajax({
          url: $('base').attr('href') + 'personajuridica/delete_contact',
          type: 'POST',
